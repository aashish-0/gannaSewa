...
import { causes as fallback } from "../data/mock";

const CausesSection = () => {
  const items = useCollection("causes", fallback);
  const home = useDoc("content/home", {});
  const eyebrow = home.causesEyebrow || "What We Do";
  const heading = home.causesHeading || "Building Lives Through Sustainable Development";
  const subtitle = home.causesSubtitle || "Five focus areas working together to move families from dependency to dignity.";
          ...
          {items.map((c) => (
            <Link key={c.id} to={c.link || "#"} className="cause-card flex flex-col items-center text-center group">
              <div className="cause-icon w-28 h-28 md:w-32 md:h-32 flex items-center justify-center mb-4">
                {c.icon && <img src={c.icon} alt={c.name} className="w-full h-full object-contain" />}
              </div>
