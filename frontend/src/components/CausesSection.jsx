        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 md:gap-6 mt-12">
          {causes.map((c) => (
            <a
              key={c.id}
              href={c.link}
              className="cause-card flex flex-col items-center text-center group"
            >
              ...
              <h4 className="text-lg md:text-xl font-semibold text-gray-800 group-hover:text-[#ec008c] transition-colors">
                {c.name}
              </h4>
            </a>
          ))}
