              ...
              <div key={j.id} className="bg-white rounded-lg shadow-md p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border-l-4 border-[#ec008c] hover:shadow-xl transition-shadow">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mb-2">
                    <span className="flex items-center gap-1"><MapPin className="w-4 h-4 text-[#ec008c]" /> {j.location}</span>
                    <span className="flex items-center gap-1"><Briefcase className="w-4 h-4 text-[#ec008c]" /> {j.type}</span>
                  </div>
                  <p className="text-sm text-gray-600">{j.desc}</p>
                </div>
                <button onClick={() => apply(j.title)} className="flex-shrink-0 inline-flex items-center gap-2 bg-[#ec008c] hover:bg-[#c70074] text-white px-6 py-2.5 rounded-full font-semibold text-sm">
