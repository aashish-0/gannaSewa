      ...
      <section className="py-16">
        <div className="container-x max-w-4xl">
          ...
          <div className="space-y-4">
            {items.map((p) => (
              <a key={p.id} href={p.link || "#"} target={p.link ? "_blank" : undefined} rel="noreferrer" className="flex items-start gap-4 bg-white p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow border-l-4 border-[#ec008c] group">
                <div className="flex-shrink-0 w-12 h-12 rounded-full bg-[#fce4f0] flex items-center justify-center"><Newspaper className="w-6 h-6 text-[#ec008c]" /></div>
